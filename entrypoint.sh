...
if [ "${INPUT_ENABLED}" = true ]; then
  ...
  enabled=false
elif echo "${message}" | grep -q "${INPUT_ENABLED}"; then
...
# Setup plugin repo
echo "> Setting up ${INPUT_PR_REPOSITORY}"
...
if "${testing}"; then
  ...
  # If committing to prod, remove the testing plugin
...
echo "> Creating PR"
gh pr create --repo "${INPUT_PR_REPOSITORY}" --title "${prTitle}" --body "${prBody}"
